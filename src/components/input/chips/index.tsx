...
const WmChips = React.memo(
  (props: WmChipsProps) => {
    ...
    const backspaceArmedRef = useRef<boolean>(false);
    ...
    const handleSearchKeyDown = useCallback(
      (event: React.KeyboardEvent) => {
        ...
        if (event.key === "Backspace") {
          ...
          const currentQuery = (inputElement?.value ?? searchQuery ?? "").trim();
          if (!readonly && !disabled && currentQuery === "" && chipsList.length > 0) {
            if (!backspaceArmedRef.current) {
              ...
              removeItem(event as unknown as React.MouseEvent, chipsList[lastIndex], lastIndex);
              backspaceArmedRef.current = false;
              return;
            }
          }
    ...
    const renderSearchInput = () => {
      return (
        <Box component="li" className={clsx("app-chip-search", { "full-width": isFullWidth })}>
          <WmSearch
  ...
  (prev, next) => {
    const keys: (keyof WmChipsProps | "hidden" | "children")[] = [
      "allowonlyselect",
      "autofocus",
      "chipclass",
      "className",
    ...
    return keys.every(key => {
      return isEqual(prev[key as any], next[key as any]);
    });
  }
);
